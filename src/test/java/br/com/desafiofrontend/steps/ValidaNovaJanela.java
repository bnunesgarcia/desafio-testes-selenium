...
public class ValidaNovaJanela extends Utils {
    ...
    public void oBotaoDeveEstaDisponivel(String botao) {
        if(botao.equals("new window")){
            RunCucumberTest.driver.findElement(By.xpath(Variaveis.newWindowBtn)).isDisplayed();
        } else if (botao.equals("add")){
            RunCucumberTest.driver.findElement(By.xpath("//*[@id=\"addNewRecordButton\"]")).isDisplayed();
        }
    }
    ...
    public void euClicoNoBotao(String botao) {
        if(botao.equals("new window")){
            RunCucumberTest.driver.findElement(By.xpath(Variaveis.newWindowBtn)).click();
        } else if (botao.equals("add")){
            RunCucumberTest.driver.findElement(By.xpath("//*[@id=\"addNewRecordButton\"]")).click();
        }
    }

    @E("uma nova tela de navegador abre")
    public void umaNovaTelaDeNavegadorAbre() {
        for (String handle : RunCucumberTest.driver.getWindowHandles()) {
            RunCucumberTest.driver.switchTo().window(handle);
        }
    }

    @E("eu valido a mensagem da tela")
    public void euValidoAMensagemDaTela() {
        String mensagem = RunCucumberTest.driver.findElement(By.xpath("//*[@id=\"sampleHeading\"]")).getText();
